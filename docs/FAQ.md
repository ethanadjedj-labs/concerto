...
Claude Max ($100/month) gives you the highest usage limits and access to the most capable models. Concerto works best with Claude Max because long-horizon agentic tasks consume more tokens than light chatting. That said, Concerto itself does not require Max — see the next question.
...
Yes. Your Droplet lives in *your* DigitalOcean account. You can destroy it at any time via the DO dashboard. We'll also destroy it on your behalf if you request it via the Concerto dashboard. Destroying the Droplet stops all DigitalOcean charges immediately.

---

### 6. Will I be charged again?

Concerto's $99 is a one-time fee. We do not charge you again unless you provision a second Droplet. Your ongoing cost is the DigitalOcean Droplet (currently $12/month for the default 2 vCPU / 4 GB RAM spec), billed directly by DigitalOcean to whatever card you have on file with them.
...
3. Contact **support@concerto.run** — we'll update the stored key and re-verify your Droplet is still under your control.
...
If provisioning fails (DO API rejection, Droplet boot failure), we offer a full refund automatically — no questions asked. If provisioning succeeds but you change your mind within 48 hours and haven't used the Droplet, email **support@concerto.run** for a full refund. After 48 hours or after first use, refunds are at our discretion. We're a small team and we want to be fair — reach out and we'll work something out.

---

### 10. Won't Anthropic ship this themselves?

Possibly. Anthropic is building operator infrastructure. When and if they do, we'll adapt. In the meantime: Concerto is available today, runs on hardware you own, and doesn't require any Anthropic-side changes. We're not betting against Anthropic — we're filling the gap that exists right now.
