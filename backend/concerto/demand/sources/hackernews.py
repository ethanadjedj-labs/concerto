...
def _hit_to_opp(hit: dict, fetched_ts: int) -> Opportunity | None:
    ...
    is_comment = "comment" in tags or (
        hit.get("comment_text") and not hit.get("story_text") and not hit.get("title")
    )
    story_title = hit.get("title") or hit.get("story_title") or ""
    ...
    kind = next((t for t in tags if t in ("story", "comment", "show_hn", "ask_hn")), None)
    if kind:
    ...
    # Build a title the operator can actually act on:
    # - Stories keep their own title.
    # - Comments get "[comment in <parent>] <snippet>" so the operator sees
    #   what they're engaging with, not just the omnibus parent thread.
    if is_comment:
        snippet_src = body.strip().splitlines()[0] if body.strip() else ""
        snippet = re.sub(r"<[^>]+>", "", snippet_src)[:120]
        parent = story_title or f"HN item {object_id}"
        title = f"[comment in: {parent}] {snippet}".strip()
    else:
        title = story_title
