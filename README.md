...
3. **Install** — `cloud-init` runs the Concerto installer: installs Claude Code, sets up an MCP server, starts a cloudflared tunnel, locks the system so only the tunnel reaches the machine.
...
1. Go to **[concerto.run](https://concerto.run)** and click **Get Started**.
...
Need help? Join the [Discord community](https://discord.gg/concerto) — real humans, fast replies.
...
installer/  — Bash installer delivered via install.concerto.run (cloud-init)
